...
const PRIORITY_COLORS = {
    LOW: "text-red-600 bg-red-200 dark:text-red-400 dark:bg-red-500/15",
    MEDIUM: "text-blue-600 bg-blue-200 dark:text-indigo-400 dark:bg-indigo-500/15",
    HIGH: "text-emerald-600 bg-emerald-200 dark:text-emerald-400 dark:bg-emerald-500/15",
};
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {metrics.map((m, i) => (
                    <div
                        key={i}
                        className="not-dark:bg-theme-card border border-theme-border rounded-xl border border-theme-border p-6 transition-all duration-200 hover:scale-[1.02]"
                    >
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-theme-text-sub text-sm">{m.label}</p>
            ...
            <div className="grid lg:grid-cols-2 gap-6">
                {/* Tasks by Status */}
                <div className="not-dark:bg-theme-card border border-theme-border rounded-xl border border-theme-border p-6">
                    <h2 className="text-theme-text mb-4 font-medium">Tasks by Status</h2>
                ...
                {/* Tasks by Type */}
                <div className="not-dark:bg-theme-card border border-theme-border rounded-xl border border-theme-border p-6">
                    <h2 className="text-theme-text mb-4 font-medium">Tasks by Type</h2>
                    <ResponsiveContainer width="100%" height={300}>
                        <PieChart>
                            <Pie
                                data={typeData}
            ...
            {/* Priority Breakdown */}
            <div className="not-dark:bg-theme-card border border-theme-border rounded-xl border border-theme-border p-6">
                <h2 className="text-theme-text mb-4 font-medium">Tasks by Priority</h2>
                <div className="space-y-4">
                    {priorityData.map((p) => (
                        <div key={p.name} className="space-y-2">
                            <div className="flex justify-between items-center">
                                <div className="flex items-center gap-2">
                                    <ArrowRightIcon className={`size-3.5 ${PRIORITY_COLORS[p.name]} bg-transparent bg-transparent`} />
                                    <span className="text-theme-text capitalize">{p.name.toLowerCase()}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <span className="text-theme-text-sub text-sm">{p.value} tasks</span>
                                    <span className="px-2 py-0.5 border border-zinc-400 dark:border-zinc-700 text-theme-text-sub text-xs rounded">
                                        {p.percentage}%
                                    </span>
                                </div>
                            </div>
                            <div className="w-full bg-theme-surface dark:bg-theme-card/[0.06] rounded-full h-1.5">
                                <div
